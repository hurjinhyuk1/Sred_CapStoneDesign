...
def read_spect_label(img_list_file) :
	...
	# img_ndarr = np.array([np.array(Image.open(img_path).convert('L')) for img_path in img_path_list])	### gray (waveform)

	# img_ndarr = img_ndarr / img_ndarr.max() -1
	img_ndarr = abs(img_ndarr / img_ndarr.max() - 1)

	img_ndarr = img_ndarr.reshape(-1, 256, 256, 3)	# png 크기에 따라 설정	### color (spectrogram)
	# img_ndarr = img_ndarr.reshape(-1, 256, 256, 1)	# png 크기에 따라 설정	### gray (waveform)
...
# spect_train_imgs, spect_train_labels = read_spect_label('png_spectrogram/spectrogram_label_train.txt')
# spect_test_imgs, spect_test_labels = read_spect_label('png_spectrogram/spectrogram_label_test.txt')

spect_train_imgs, spect_train_labels = read_spect_label('png_waveform/waveform_label_train.txt')
spect_test_imgs, spect_test_labels = read_spect_label('png_waveform/waveform_label_test.txt')
...
X = tf.placeholder(tf.float32, [None, 256, 256, 3], name='X')	### color (spectrogram)
# X = tf.placeholder(tf.float32, [None, 256, 256, 1], name='X')	### gray (waveform)
...
K = 6		# 1st convolutional layer output depth
L = 12		# 2nd convolutional layer output depth
M = 24		# 3rd
N = 48		# 4th
O = 6144	# fully connected layer


# 가중치 W, B 설정

W1 = tf.Variable(tf.truncated_normal([5, 5, 3, K], stddev = 0.1))	# 5x5 patch, 3 input channels, K output channels	### color (spectrogram)
# W1 = tf.Variable(tf.truncated_normal([5, 5, 1, K], stddev = 0.1))	# 5x5 patch, 3 input channels, K output channels	### gray (waveform)
...
W3 = tf.Variable(tf.truncated_normal([4, 4, L, M], stddev = 0.1))	# 4x4 patch, L input channels, M output channels
B3 = tf.Variable(tf.ones([M])/10)
W4 = tf.Variable(tf.truncated_normal([4, 4, M, N], stddev = 0.1))	# 4x4 patch, M input channels, N output channels
B4 = tf.Variable(tf.ones([N])/10)

W5 = tf.Variable(tf.truncated_normal([16 * 16 * N, O], stddev = 0.1))
B5 = tf.Variable(tf.ones([O])/10)

W6 = tf.Variable(tf.truncated_normal([O, 10], stddev = 0.1))
...
# The model
stride = 1
Y1 = tf.nn.relu(tf.nn.conv2d(X, W1, strides=[1, stride, stride, 1], padding = 'SAME') + B1)		# conv 256x256 >> 256x256
# Y1 = tf.nn.max_pool(Y1, ksize=[1,2,2,1], strides=[1,2,2,1], padding = 'SAME')					# pool 256x256 >> 128x128

stride = 2	# 256 >> 128
Y2 = tf.nn.relu(tf.nn.conv2d(Y1, W2, strides=[1, stride, stride, 1], padding = 'SAME') + B2)	# conv 128x128 >> 128x128
Y2 = tf.nn.max_pool(Y2, ksize=[1,2,2,1], strides=[1,2,2,1], padding = 'SAME')					# pool 128x128 >> 64x64

stride = 1
Y3 = tf.nn.relu(tf.nn.conv2d(Y2, W3, strides=[1, stride, stride, 1], padding = 'SAME') + B3)	# conv 64x64 >> 64x64
# Y3 = tf.nn.max_pool(Y3, ksize=[1,2,2,1], strides=[1,2,2,1], padding = 'SAME')					# pool 64x64 >> 32x32

stride = 2	# 64 >> 32
Y4 = tf.nn.relu(tf.nn.conv2d(Y3, W4, strides=[1, stride, stride, 1], padding = 'SAME') + B4)	# conv 32x32 >> 32x32
Y4 = tf.nn.max_pool(Y4, ksize=[1,2,2,1], strides=[1,2,2,1], padding = 'SAME')					# pool 32x32 >> 16x16

# reshape the output from the 3rd convolution for the fully connected layer
YY = tf.reshape(Y4, shape=[-1, 16 * 16 * N])	# 최종 이미지 크기에 맞춰 재설정
...
for i in range(l_times + 1) :
	...
	# save model
	if i % 50 == 0 :
		saver.save(sess, './model_sred/sred_model', global_step=i)